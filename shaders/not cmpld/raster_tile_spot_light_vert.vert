...
vec3 spotVertexTransform(UnifiedLightData light, uint transformType, vec3 transformDirection)
{
	...
	if (transformType != NO_TRANSFORM)
	{
		...
		{
			resPos = transformDirection * sinTheta * 2.0;
			resPos.y += cosTheta;
		}
		float cosPhi = light.direction.y;
		float sinPhi = sqrt(1 - cosPhi * cosPhi);
		
		vec3 rotAxis = (abs(light.direction.z) < 0.001 && abs(light.direction.x) < 0.001) ? vec3(1.0, 0.0, 0.0) : normalize(vec3(light.direction.z, 0.0, -light.direction.x));
...
void main()
{
	...
	* light.lightLength + light.position, 1.0);
}