...
#define RGBA_LE(col) (((col & 0xff000000) >> (3 * 8)) + ((col & 0x00ff0000) >> (1 * 8)) + ((col & 0x0000ff00) << (1 * 8)) + ((col & 0x000000ff) << (3 * 8)))
    ...
    const static uint32_t wetAsphalt = RGBA_LE(0x34495effu);
    const static uint32_t midnightBlue = RGBA_LE(0x2c3e50ffu);

    const static uint32_t concrete = RGBA_LE(0x95a5a6ffu);
    const static uint32_t asbestos = RGBA_LE(0x7f8c8dffu);
